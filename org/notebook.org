...
** DONE Test 3 available until March 10 / grading
...
** IN PROGRESS Working with shell commands - 60-75 min (code along)
...
** NEXT Review: The Linux File System and Shell Commands
...
** TODO Lecture: Know your Pi (Part 1)
...
** NEXT Review - redirection

1. What does the ~rev~ command do? (Reverses lines characterwise)
   #+begin_quote
   - The ~rev~ command reverses the characters in each line of a file
     or standard input.
   #+end_quote
...
2. How can you redirect the standard output of a command to a file,
   ...
   - You can redirect the standard output using the ~>~ operator, as in ~ls
     -l /usr/bin > ls-output.txt~. By default, an ASCII text file is
     created.
   #+end_quote

3. What is the effect of executing ~ls -l /bin/usr > ls-output.txt~ when
   the ~/bin/usr~ directory does not exist, and how does redirecting
   standard error (stderr) differ from redirecting standard output
   (stdout)?
   #+begin_quote
   - When the directory does not exist, the standard error message is not
     redirected to ~ls-output.txt~ but displayed on the screen instead. To
     redirect stderr, use ~2>~ operator, as in ~ls -l /bin/usr 2>
     ls-error.txt~.
   #+end_quote
...
4. Describe the process and outcome of appending both standard output
   ...
   - To append both stdout and stderr to the same file, use the ~&>>~
     operator for appending, as in ~ls -l /bin/usr &>> ls-output.txt~. This
     appends all output, including errors, to the specified file.
   #+end_quote
...
5. How can you create a new empty file or discard command output using
   ...
   - You can create an empty file by using ~> filename~ with no source
     command, and you can discard any command output by redirecting it to
     ~/dev/null~, as in ~command > /dev/null~ for stdout or ~command 2>
     /dev/null~ for stderr.
   #+end_quote

** Expansion

- Lecture: 5_expansion.org in GitHub
- Codealong: tinyurl.ncom/4-expansion-org

* Week 10 - Your Pi / General Purpose Input Output (GPIO) pins
