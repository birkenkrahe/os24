...
1) Which information is not kept as a file in Linux?
   ...
   - Kernel data outside of ~/proc~ and ~/sys~
   #+end_quote
...
2) How are "special files" identified, and what are examples?
   ...
   - Symbolic (soft) links identified by `c` at the start of `ls -l` output
   - Block device files identified by `c` at the start of `ls -l` output
