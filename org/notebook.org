...
- [ ] Practice: Get started with Raspberry Pi: connect and update (Thursday)
- [ ] DataCamp review/Practice: Manipulating files and directories (Thursday)
- [ ] Assignment: Discuss "A practical view of the Linux system" (Thursday)
...
** IN PROGRESS DataCamp lesson review
...
** IN PROGRESS Lecture: OS fundamentals

- [[file:1_fundamentals.org]] - Part 1: From kernel to timesharing.

** TODO Practice: first use of the shell on Raspberry Pi

[[file:1_fundamentals_practice.org]]
...
- Download the ~.emacs~ configuration file:
  ...
  2) Enter the URL ~tinyurl.com/EmacsLyon~
  3) Write the file to ~~/.emacs~ with ~C-x C-w~
  4) Download the instructions from ~tinyurl.com/TermEmulator~
  5) Write the file to ~/1_fundamentals_practice.org~
  6) View your home directory with ~C-x d~ and enter ~s~ to see both files.
  7) Quit Emacs with ~C-x C-c~
  8) Fire Emacs back up again from the shell with ~emacs &~
  9) Work through the instructions.
  10) When finished, close down your computer with ~sudo shutdown now~
...
- [[file:1_fundamentals.org]] - Part 2: From virtualization to FOSS
